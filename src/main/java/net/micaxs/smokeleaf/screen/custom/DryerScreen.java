...
public class DryerScreen extends AbstractContainerScreen<DryerMenu> {
    ...
    public static final ResourceLocation GUI_TEXTURE = ResourceLocation.fromNamespaceAndPath(SmokeleafIndustries.MODID, "textures/gui/dryer/dryer_gui.png");
    ...
    private void renderInfoIconTooltip(GuiGraphics g, int mouseX, int mouseY, int baseX, int baseY) {
        if (isMouseAboveArea(mouseX, mouseY, baseX, baseY, ICON_SIZE, ICON_SIZE, ICON_SIZE, ICON_SIZE)) {
            ...
            g.renderTooltip(
                    this.font,
                    List.of(Component.translatable("gui.tooltip.dryer.info")),
                    Optional.empty(),
                    (mouseX / 2) - (baseX / 4),
                    mouseY / 2
            );
    ...
    private void renderProgressArrow(GuiGraphics guiGraphics, int x, int y) {
        if(menu.isCrafting()) {
            guiGraphics.blit(GUI_TEXTURE, x + 70, y + 30, 0, 166, menu.getScaledProgress(), 21);
        }
    }
